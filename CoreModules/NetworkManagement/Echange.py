...
def encode_update_packets(update: LogicUpdate): #-> List[Packet]
    ...
    while sum(len(update_element) for update_element, _ in update_elements) > 0:
        ...
        packets.append(
            Packet(bytearray(packetBody), 9200, "192.168.241.176",
                   "192.168.241.154", final=False, packetType=PacketTypes.Update)
        )
    if len(packets) > 0:
...
if __name__ == "__main__":
    p = Packet(b"test", 9200, "127.0.0.1",
               "127.0.0.1", PacketTypes.Default, True)
    print(p)
    print(p.pack())
